...
namespace HadChi2
{
  ...
  public :
    ...
    double GetChi2( const std::vector<TLorentzVector>& jets, const std::vector<float>& bDiscs, bool isData);
    ...
    const double GetSigmaRecoTop(){ return smtop_;}
    
    // Mass of reconstructed top
    const double GetMRecoTop(){ return mtop_;}    
    
    // LorentzVector of reconstructed top
    const TLorentzVector GetTLVRecoTop(){ return hadT_;}
