...
int TagEff::JetLoop(vector<Jet*>& pureJets) //Takes the Jet collection (in any step of cut flow) obtain eff. maps or weights
{
  ...
  if(status) ApplySF(pureJets, &nbjet);
	...
	{
	  tagflavor(pureJets.at(njet)->PartonFlavor());
  ...
  for(int njet = 0; njet < pureJets.size(); njet++)    
    {
      ...
      sf = SF(Pt, fabs(pureJets.at(njet)->eta()), pureJets.at(njet)->PartonFlavor());
      //sf=1.;
      if("b-matched"==matchtype)  {
...
double TagEff::SF( double pt, double eta, int flavor)
{
  return desy_tools::GetBJetSF(pt, eta, "CSV", "M", flavor);
}
